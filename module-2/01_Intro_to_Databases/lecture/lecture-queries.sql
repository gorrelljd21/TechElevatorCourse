...
SELECT * FROM state; --show everything if you don't know the column/row names, build up a statement, start wide and then narrow down
SELECT state_name FROM state WHERE census_region IS NULL; --where census region is "unkown"
...
SELECT city_name FROM city; -- only one printed out of duplicates

-- Do it again, but without the repeats (note the difference in row count)
SELECT DISTINCT city_name, state_abbreviation FROM city;
SELECT DISTINCT city_name, state_abbreviation FROM city ORDER BY city_name;
...
SELECT city_name FROM city WHERE city_name LIKE '%Falls'; --ILIKE ignores case
SELECT city_name FROM city WHERE city_name LIKE 'C_n%'; -- to get single character wildcard, would print 'Cincinnati'

-- The names of all cities that contain a space
SELECT city_name FROM city WHERE city_name LIKE '% %';  --ILIKE ignores case
--use NOT LIKE to get all cities without a space

-- BETWEEN
-- Write a query to retrieve the names and areas of parks of at least 100 sq. kilometers but no more than 200 sq. kilometers
SELECT park_name, area FROM park WHERE area >= 100 AND area <= 200; -- no BETWEEN used
SELECT park_name, area FROM park WHERE area BETWEEN 100 AND 200; --has to go low to high, dates earliest to latest
