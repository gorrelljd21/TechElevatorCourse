...
public class AuctionService {
    ...
    public Auction add(Auction newAuction) {
        HttpHeaders headers = new HttpHeaders(); // set the header
        headers.setContentType(MediaType.APPLICATION_JSON); //setting content type to JSON a media type
        HttpEntity<Auction> httpEntity = new HttpEntity<>(newAuction, headers); //connecting the body/input to the headers

        Auction result = null;

        try {
            result = restTemplate.postForObject(API_BASE_URL, httpEntity, Auction.class); //sets the verb as POST
