...
function lastDigit(x, y) {
	...
	const xLast = xToString.substr(-1);
	const yLast = yToString.substr(-1);
...
function frontAgain(string) {
	...
	frontTwo = string.substring(0, 2);
	lastTwo = string.substr(-2);
...
function cigarParty(cigars, isWeekday) {

	if ((cigars >= 40 && cigars <= 60) && isWeekday == false) {
		return true;
	} else if (cigars > 40 && isWeekday == true) {
		return true;
	} else {
		return false;
	}
...
function filterBigNumbers(array) {
	...
	for (let i = 0; i < array.length; i++) {
		if (array[i] >= 100) {
			bigNumberedArray.push(array[i]);
		}
	}
	return bigNumberedArray;
}
